...
//            put( CharSequence.class.getName(), true );
    ...
    public static void main( String args[] ) {
        ...
        String knownFrameworkOrder[] = ("java_swift|"+args[0]).split("\\|");
    ...
    void generateCallbackBase( HashMap<String,Boolean> fieldsSeen, java.lang.reflect.Method responsibles[] ) throws IOException {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if ( notVoid(returnType) ) {
                ...
                code.append("    return JNI.check( "+(!isObjectType( returnType ) || true ? //returnType.isPrimitive() || returnType == java.lang.String.class ?
                        encoder("__return", returnType, "&locals") + encodeSuffix(returnType) :
                        "/*JNI.api.NewWeakGlobalRef( JNI.env,*/ __return?.takeJavaObject /*)*/")+", &locals, removeLast: true )\n");
            }
   ...
   boolean ambiguousInitialiser( String signature ) {
    ...
    String encoder( String var, Class<?> type, String locals ) {
        ...
        return "JNIType.toJava( value: "+var+(isObjectType( type ) &&
        		!type.isInterface() && !java.util.Map.class.isAssignableFrom(type) ?
        		" != nil ? "+var+"! as JNIObject : nil":"")+
        		(java.util.Map.class.isAssignableFrom(type) ?
        				", mapClass: \""+type.getName().replace(".", "/")+"\"":"") +", locals: "+locals+" )";
    }
